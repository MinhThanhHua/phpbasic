function checkFormUser() {
    ...
    if (name.length < 3) {
        $('.show').eq(0).text('Tên quá ngắn').css({'color':'red'});
        return false;
    }
    $('.show').eq(0).text('');
    ...
    if (address.length < 10 || address.length > 150) {
        $('.show').eq(3).text('Địa chỉ từ 10 đến 150 ký tự').css({'color':'red'});
        return false;
    }
    $('.show').eq(3).text('');
    ...
    if (phone.length < 8 || phone.length > 10) {
        $('.show').eq(4).text('Điện thoại là số từ 8 đến 10 ký tự').css({'color':'red'});
        return false;
    }
    $('.show').eq(4).text('');
    if (isNaN(phone) || (phone) <= 0) {
        $('.show').eq(4).text('Chỉ được điền số > 0').css({'color':'red'});
        return false;
    }
    $('.show').eq(4).text('');
    return true;
